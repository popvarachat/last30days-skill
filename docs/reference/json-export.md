...
| Field | Type | Meaning |
| --- | --- | --- |
| `schema_version` | string | Agent export contract version. The current version is `1.5`. |
...
| `topic_relevance_score` | number | Query-centric topical relevance before global reranking, normalized to `0.0`-`1.0`. Added in `1.5`. |
| `topic_anchor_score` | number | Coverage of distinctive topic anchors after generic AI/workflow terms are discounted; omitted without session personalization. Added in `1.5`. |
| `personal_relevance_score` | number | Session-scoped personal-interest match normalized to `0.0`–`1.0`; omitted when no personal context was supplied. The underlying interest terms are never exported. Added in `1.4`. |
| `curator_score` | number | Personalized decision score combining topic fit, distinctive topic anchors, personal fit, freshness, and source quality; omitted without session personalization. Added in `1.5`. |
| `recommendation_reason` | string | Coarse explanation of topic/personal fit. It never includes the supplied interest terms. Added in `1.5`. |
...
```json
{
  ...
  "reports": [
    {"entity": "OpenAI", "report": {"schema_version": "1.5", "query": "OpenAI"}},
    {"entity": "Anthropic", "report": {"schema_version": "1.5", "query": "Anthropic"}}
  ]
}
```
...
- `1.5` added topic-aware curator scoring fields (`topic_relevance_score`, optional `topic_anchor_score`, optional `curator_score`, and `recommendation_reason`) so personal matches cannot outrun weak topic anchors. Interest terms remain private.
- `1.4` added optional `personal_relevance_score` to each result when session-scoped interest context influenced ranking. Interest terms themselves are not exported.
